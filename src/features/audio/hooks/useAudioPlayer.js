...
export const useAudioPlayer = (audioUrl, albumTracks = null, currentTrackIndex = 0, onTrackChange = null, autoplayEnabled = true) => {
  ...
  useEffect(() => {
    ...
    if (audioUrl && fastMetadataService.isInitialized) {
      const fileName = extractFileNameFromUrl(audioUrl);
      ...
      log.audio(`FastMetadataService not initialized yet for ${audioUrl}`);
    }
    ...
    if (durationFromMetadata) {
      ...
      if (audioUrl && !fastMetadataService.isInitialized && !fastMetadataService.isLoading) {
        const fileName = extractFileNameFromUrl(audioUrl);
        if (fileName) {
          log.audio(`Trying to initialize FastMetadataService for ${fileName}...`);
          fastMetadataService.initialize().then(() => {
            ...
            log.audio(`Failed to initialize FastMetadataService: ${error.message}`);
          });
        }
      }
    }
    ...
    const updateDuration = () => {
      ...
      if (canAutoplay) {
        ...
        const proceedWithAutoplay = () => {
          ...
          // ODSTRANĚN setTimeout - na Androidu to přerušuje kontext uživatelského gesta a způsobuje NotAllowedError
          audio.play().then(() => {
            setAudioState(prev => ({ ...prev, isPlaying: true, userPaused: false }));
        ...
        if (audio.readyState >= 2 && audio.networkState !== 3) {
          ...
          try {
            ...
            if (audioContext.state === 'suspended') {
              ...
              // ODSTRANĚN .then() aby se audio.play() zavolal synchronně (nutné pro mobilní prohlížeče)
              audioContext.resume().catch((error) => {
                console.log('🎵 AUTOPLAY: Failed to resume AudioContext:', error);
              });
            }
            
            console.log('🎵 AUTOPLAY: AudioContext is active (or resuming), proceeding with autoplay');
            window.audioActivated = true;
            proceedWithAutoplay();
          } catch (error) {
