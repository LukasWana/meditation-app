...
// Adapted for Shader Sequencer by AI with GLSL fixes and audio reactivity.
...
float blobs(vec2 p) {
  ...
  float dblobs = 1E6;
  for (int i = 0; i < 5; ++i) {
    // Audio: Bass pulses blob size
    float blob_size = 0.1 * (1.0 + iAudio.x * 2.0);
    float dd = circle(bp-1.0*vec2(sin(TIME+float(i)), sin(float(i*i)+TIME*sqrt(0.5))), blob_size);
...
vec3 skyColor(vec3 ro, vec3 rd) {
  ...
  vec3 gcol = HSV2RGB(vec3(0.45 + iAudio.y * 0.2, 0.6, 1.0));
...
vec3 render1(vec3 ro, vec3 rd) {
  ...
  for (int bounce = 0; bounce < MAX_BOUNCES; ++bounce) {
    ...
    vec3 gcol = ragg*lcol*exp(8.0*(min(de-0.2, 0.0))) * (1.0 + iAudio.z * 1.5);
    // Will never miss
    if (de < 0.0) {
      agg += gcol;
...
vec3 effect(vec2 p) {
  ...
  // Audio: Overall volume controls camera speed
  float a = 0.5*(-0.5+0.5*sin(0.123*TIME * (1.0 + iAudio.w * 0.5)));
  float b = 0.1*TIME * (1.0 + iAudio.w * 0.5);
  if (iMouse.x > 0.0) {
