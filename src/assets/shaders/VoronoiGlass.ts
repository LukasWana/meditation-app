...
vec4 alphaBlend(vec4 back, vec4 front) {
  ...
  vec3 xyz = (front.xyz*front.w + back.xyz*back.w*(1.0-front.w))/(w + 1e-6); // Epsilon added
...
vec3 skyColor(vec3 ro, vec3 rd) {
  ...
  vec3 scol = HSV2RGB(vec3(0.1 + iAudio.y*0.2, 0.25, 0.9));
...
float hf2(vec2 p) {
  float zoomOuter = 1.0 + iAudio.x;
  float zoomInner = 0.2 + iAudio.x * 0.1;
  
  p /= zoomOuter;
  p /= zoomInner;
  
  float d = -voronoi2(p);
  d *= zoomInner*zoomOuter;
  
  float h = 0.2*tanh_approx(3.0*smoothstep(0.0, 1.0*zoomOuter*zoomInner, -d));
  
  return h*zoomOuter*zoomInner;
}

float height(vec2 p, float hmul) {
  return -hf2(p)*hmul;
}

vec3 normal(vec2 p, float eps, float hmul) {
  vec2 e = vec2(0.00001, 0);
  
  vec3 n;
  n.x = height(p + e.xy, hmul) - height(p - e.xy, hmul);
  n.y = height(p + e.yx, hmul) - height(p - e.yx, hmul);
  n.z = -2.0*e.x;
  
  return normalize(n);
}

// Using mat4 to return both color and normal to avoid 'out' parameters.
// result[0] is color (vec4), result[1].xyz is normal (vec3).
mat4 plane(vec3 pro, vec3 ro, vec3 rd, vec3 pp, vec3 off, float aa, float n_) {
  float h0 = hash(n_);
  ...
  float l_hmul = smoothstep(0.0, 0.125, (hd-hsz/2.0));

  p += vec2(h0,h1)*20.0;
  p *= mix(0.5, 1.0, h2);
  float he  = height(p, l_hmul);
  vec3  nor = normal(p,2.0*aa, l_hmul);
  vec3 po   = pp;
  ...
  float spes = 20.0 + iAudio.z * 30.0; // Highs affect specular
  ...
  return mat4(vec4(col, t), vec4(pnor, 0.0), vec4(0.0), vec4(0.0));
}
...
vec3 color(vec3 ww, vec3 uu, vec3 vv, vec3 pro, vec3 ro, vec2 p) {
  ...
  const int fadeFrom = 3; // max(furthest-2, 0)
  ...
  for (int i = 1; i <= furthest; ++i) {
    ...
    if (pd > 0.0 && acol.w < cutOff) {
      ...
      mat4 planeResult = plane(pro, ro, rd, pp, off, aa, nz+float(i));
      vec4 pcol = planeResult[0];
      vec3 pnor = vec3(planeResult[1]);
...
vec3 effect(vec2 p, vec2 q) {
  float z   = 0.33*planeDist*iTime; // Wrapper defines iTime, which is audio-reactive via u_speed
