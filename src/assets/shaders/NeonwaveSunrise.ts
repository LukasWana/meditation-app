...
// FIX: Removed 'const' because it's assigned to in some GLSL versions. Global variables are better as non-const.
...
vec3 stars(vec2 sp, float hh) {
  // FIX: Removed 'const' because it's initialized with a non-constant expression (a macro with a function call).
  ...
  for (int i_int = 0; i_int < 6; ++i_int) {
    ...
    // Audio: Highs make stars twinkle
...
vec4 moon(vec3 ro, vec3 rd) {
  ...
  for (int i = -limit; i <= limit; ++i) {
    ...
    if (i == 0) fft = iAudio.y;  // mid
    if (i == 1) fft = iAudio.z;  // high

    vec2 cp     = vec2(0.055*abs(msny-fi), 0.25);
...
vec3 skyColor(vec3 ro, vec3 rd) {
  ...
  vec3 lpos   = 1E6*vec3(iAudio.x * 0.1, -0.15, 1.0); // Audio: Bass moves sun
  ...
  // FIX: Renamed 'li' to 'lightIntensity' to resolve a compiler error where the name 'li' was not found.
  float lightIntensity    = 0.02*mix(1.0, 10.0, lf)/(abs((rd.y+0.055))+0.025);
...
vec3 color(vec3 ww, vec3 uu, vec3 vv, vec3 ro, vec2 p) {
  ...
  // FIX: max() is not for integers in GLSL ES 1.0. Hardcoded the result.
...
vec3 aces_approx(vec3 v) {
  v = max(v, 0.0);
  // FIX: Removed 'f' suffix from floats for GLSL ES 1.0 compatibility.
  v *= 0.6;
  float a = 2.51;
  float b = 0.03;
  float c = 2.43;
