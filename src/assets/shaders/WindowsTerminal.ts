...
vec3 render0(vec3 ro, vec3 rd) {
  vec3 col = vec3(0.0);
  vec3 sunDir1    = normalize(vec3(0., 0.05, -1.0));

  vec3 skyCol     = HSV2RGB(vec3(hoff+0.50 + iAudio.y * 0.1, 0.90, 0.25));
  vec3 skylineCol = HSV2RGB(vec3(hoff+0.70, 0.95, 0.5));
  vec3 sunCol     = HSV2RGB(vec3(hoff+0.80, 0.90, 0.5 + iAudio.x * 0.2));
...
float df(vec3 p) {
  vec3 p0 = p;
  p0.xy *= ROT(0.2*p0.z-0.1*TIME * (1.0 + iAudio.w * 0.5));
...
vec3 boxCol(vec3 col, vec3 nsp, vec3 ro, vec3 rd, vec3 nnor, vec3 nrcol) {
  ...
  vec3 diffCol1   = HSV2RGB(vec3(hoff+0.75, 0.90, 0.5));
  vec3 diffCol2   = HSV2RGB(vec3(hoff+0.95, 0.90, 0.5));

  vec3 skyCol     = HSV2RGB(vec3(hoff+0.50 + iAudio.y * 0.1, 0.90, 0.25));
  vec3 skylineCol = HSV2RGB(vec3(hoff+0.70, 0.95, 0.5));

  float lpf = 5.0;
  vec3 lightPos1  = lpf*vec3(+1.0, 2.0, 3.0);
  vec3 lightPos2  = lpf*vec3(-1.0, 2.0, 3.0);
  ...
  scol += nrcol*0.75*mix(vec3(0.25), vec3(0.5, 0.5, 1.0), nfre) * (1.0 + iAudio.z * 0.5);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  g_mod = mix(1.25, 2.5, 0.5+0.5*sin(TAU*TIME/66.0 + iAudio.x * 2.0));
  vec3 col = effect(p);
