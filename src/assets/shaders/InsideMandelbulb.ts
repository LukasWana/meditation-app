...
//  Adapted for Shader Sequencer with GLSL fixes and audio reactivity.
...
float mandelBulb(vec3 p) {
  float power = POWER + iAudio.x * 4.0;
  vec3 z  = p;
  vec3 dz = vec3(0.0);
  float r, theta, phi;
  float dr = 1.0;
...
vec3 skyColor(vec3 ro, vec3 rd) {
  vec3 skyCol     = HSV2RGB(vec3(g_hoff+0.6, 0.86, 1.0));
...
float df(vec3 p) {
  ...
  vec3 gcol = HSV2RGB(vec3(g_hoff + 0.05, 0.95, 2.0));
  vec3 beer = -gcol;
...
vec3 render(vec3 ro, vec3 rd) {
  ...
  vec3 diffuseCol = HSV2RGB(vec3(g_hoff+0.6, 0.85, 1.0));
...
vec3 effect(vec2 p) {
  float speed = 0.2 + iAudio.w * 0.4;
  g_rot = rot_x(speed*iTime)*rot_y(speed*1.5*iTime);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
  ...
  vec3 col = vec3(0.0);
  col = effect(p);
  col += col * iAudio.z * 0.5; // Treble adds shimmer
  col = aces_approx(col);
  col = sRGB(col);
