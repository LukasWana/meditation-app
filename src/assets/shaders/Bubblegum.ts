...
void main()
{
	...
	for( int i = 0; i < iter; ++i )
	{
		...
		t += (pow(max(0.0,dot(normalize(reflect(light-v, normalize(hv.xyz))), normalize(b))), 34.0) + abs(dot(normalize(hv.xyz), light-v))*0.1 * (normalize(hv.xyz)+1.0)) * pow(h, -1.2) * 0.1 * float(iter-i) * audio_mod;
	}
	t = t / float(iter*iter);
