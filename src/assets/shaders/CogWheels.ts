...
vec2 hextile(inout vec2 p) {
  ...
  // A small fix to avoid mix() with a boolean argument which is not standard in older GLSL
  float condition = float(length(p1) < length(p2));
  vec2 p3 = mix(p2, p1, condition);
...
float ccell2(vec2 p, float r) {
  ...
  // Audio: Mids make clock hands jitter
  float ctime = -(iTime*0.05 + r + iAudio.y * 2.0)*TAU;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Audio: Bass makes the scene zoom out
  float z = mix(0.5, 1.0, pcos(tm*sqrt(0.3))) * (1.0 - iAudio.x * 0.2);
  ...
  // Audio: Treble makes ripples brighter
  col += (0.4 + iAudio.z * 0.6) * pow(abs(sin(20.0*d)), 0.6);  
