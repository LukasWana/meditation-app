...
// FIX: Removed 'const' for GLSL 1.0 compatibility, as they are initialized with function calls.
vec3 skyCol     = HSV2RGB(vec3(hoff+0.6, 0.86, 1.0));
vec3 glowCol    = HSV2RGB(vec3(hoff+0.065, 0.8, 6.0));
vec3 diffuseCol = HSV2RGB(vec3(hoff+0.6, 0.85, 1.0));
...
float df(vec3 p) {
  ...
  float d = d0;
  d = max(d, -d1);
  // Audio: Mids affect refraction pattern
  float dd = smoothstep(-0.5, 0.5, sin(-iTime+(20.0*(d2 - d0) + iAudio.y*5.0)));
  vec3 mat = vec3(0.8, 0.5, mix(0.8, -1.2, dd));
  // Audio: Bass affects beer color/absorption
  vec3 beer = 1.0*vec3(0.1, 0.2, 0.05) * (1.0 + iAudio.x * 2.0);
  
  if (d3 < d) {
    d = d3;
    mat = vec3(0.5, 0.5, 0.8);
    // Audio: Highs make glow more intense
    beer = -glowCol * (1.0 + iAudio.z);
  }
...
vec3 effect(vec2 p) {
  // Audio: Overall volume affects rotation speed
