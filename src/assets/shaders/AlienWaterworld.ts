...
const source = `
// License CC0: Alien Waterworld - by inigo quilez
// Adapted for Shader Sequencer and made audio-reactive
...
const vec3  smallSunQCol2  = vec3(1.0,0.5,0.25)*0.5;
...
float loheight(vec2 p, float d) {
  const float aa   = 0.5;
  const float ff   = 2.03;
  // Make water texture swirl with mids
  float tt   = 1.3 + iAudio.y * 0.3;
  const float oo   = 0.93;
  ...
  // Make overall wave height pulse with bass
  return max(s.x+0.125, 0.0) * (0.5 + iAudio.x * 0.5);
}
...
vec3 skyColor(vec3 ro, vec3 rd) {
  ...
  vec3 sunCol1 = vec3(1.0,0.9,0.8) * (1.0 + iAudio.z * 0.3); // Pulse with treble
  vec3 sunCol2 = vec3(1.0,0.9,0.8);
  vec3 sunCol = 0.5*sunCol1*pow(sunDot, 20.0) + 8.0*sunCol2*pow(sunDot, 2000.0);
  vec3 smallSunCol = 0.5*smallSunCol1*pow(smallSunDot, 200.0) + 8.0*smallSunQCol2*pow(smallSunDot, 20000.0);
  ...
  vec3 dynPlanetCol = sqrt(vec3(0.9, 0.8, 0.7)) + iAudio.y * vec3(0.0, 0.1, 0.2); // Shift to cyan with mids
  vec3 planetCol = mix(1.3*dynPlanetCol, 0.3*dynPlanetCol, pow(psin(planetLat+1.0)*psin(sqrt(2.0)*planetLat+2.0)*psin(sqrt(3.5)*planetLat+3.0), 0.5));
  ...
  vec3 dynRingColor = sqrt(vec3(0.95, 0.65, 0.45)) + iAudio.x * vec3(0.2, 0.1, 0.0); // Glow red/orange with bass
  vec3 ringsCol = mix(vec3(0.125), 0.75*dynRingColor, ringsMix)*step(-pi, 0.0)*step(ringsDist, ringsMax)*step(-ringsDist, -ringsMin)*ringsMul;
...
vec3 shipColor(vec2 p, float time) {
  vec2 pp = toPolar(p);
  pp.y += pp.x*0.05;
  p = toRect(pp);
...
vec3 getColor(vec3 ro, vec3 rd, float time) {
  int max_iter = 0;
  ...
  if (id > 0.0) {
    ...
    // Make sea color react to bass
    seaCol.rb += iAudio.x * 0.2;
    seaCol += pow(max(dot(seaNormal, sunDir), 0.0), 2.0); 
    seaCol *= cshd;
    seaCol += 0.075*pow(vec3(0.1, 1.3, 4.0), vec3(max(dot(seaNormal, seaRef), 0.0)));

    float spe = pow(max(dot(sunDir, reflect(rd, n)), 0.0), 3.0);
    float fre = pow(1.0-dot(n, -rd), 2.0);

    col = seaCol;
    
    const float level = 0.00;
    const float level2 = 0.3;
    vec3 sunCol1 = vec3(1.0,0.9,0.8);
    ...
    // Make water glints react to treble
    col += vec3(0.5)*spe*fre * (1.0 + iAudio.z * 1.5);
    
    float ssd = (shipHeight - ro.y)/rd.y;
    
    col += shipColor((ro + rd*ssd).xz, time);
...
vec3 getSample1(vec2 p) {
  float time = mod(iTime * (1.0 + iAudio.w * 0.5), PERIOD);
