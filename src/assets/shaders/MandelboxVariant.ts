...
float mb(vec3 z) {
    ...
    float fd = 0.0;
    const float k = 0.05;
    // Audio: Bass affects fractal scale
    float final_scale = -2.8 - iAudio.x * 0.3;
    
    for(int n = 0; n < 5; ++n) {
        box_fold(k/dr, z, dr);
        sphere_fold(z, dr);
        z = final_scale * z + offset;
        dr = dr * abs(final_scale) + 1.0;        
...
vec3 render(in vec3 ro, in vec3 rd, float time_mod) {
  ...
  float h = fract(time_mod/30.0);
  
  if (t < MAX_RAY_LENGTH && pos.y > 0.0) {
    // Ray intersected object
    nor       = normal(pos);
    // Audio: Mids shift hue
    vec3 hsv  = (vec3(fract(h - 0.6 + 0.4 + 1.75*t + iAudio.y * 0.5), 1.0-ifade, 1.0));
    color     = hsv2rgb(hsv);
  } else if (pos.y > 0.0) {
    // Ray intersected sky
    return skyCol*ifade;
    ...
    vec2 pp = pos.xz*1.5;
    float m = 0.5+0.25*(sin(3.0*pp.x+time_mod*2.1)+sin(3.3*pp.y+time_mod*2.0));
    m *= m;
    m *= m;
    pp = fract(pp+0.5)-0.5;
    float dp = pmin(abs(pp.x), abs(pp.y), 0.025);
    // Audio: Highs make ground flash
    vec3 hsv = vec3(0.4+mix(0.15,0.0, m), tanh_approx(mix(100.0, 1.0, m)*dp), 1.0 + iAudio.z);
  ...
  // Audio: Highs increase specular flash
  float spe = pow(max(dot(reflect(-ld, nor), -rd), 0.0), 10.0) * (1.0 + iAudio.z * 2.0);
  float l   = dif*sha;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  float time_mod = iTime * (0.8 + iAudio.w * 0.8);

  vec3 ro = 0.4*vec3(2.0, 0, 0.2)+vec3(0.0, 1.25, 0.0);
  float tm = mix(10.33, 11.7, 0.5+0.5*sin(time_mod*TAU/60.0));
  ro.xz *= ROT(sin(tm));
  ...
  vec3 col = render(ro, rd, time_mod);
