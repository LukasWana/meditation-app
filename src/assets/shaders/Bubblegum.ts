...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for (int i = 0; i < iter; ++i)
    {
        ...
        // Audio-reactive lighting color
        vec3 audio_mod = vec3(1.0 + iAudio.x, 0.5 + iAudio.y * 2.0, 0.8 + iAudio.z);

        float spec = max(0.0, dot(normalize(reflect(light - v, normalize(hv.xyz))), normalize(b)));
        float diff = abs(dot(normalize(hv.xyz), light - v));
        t += (pow(spec, 34.0) + diff * 0.1 * (normalize(hv.xyz) + 1.0)) * pow(max(h, 1e-3), -1.2) * 0.1 * float(iter - i) * audio_mod;
    }
    t = t / float(iter * iter);
