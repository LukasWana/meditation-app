...
float rayTorus(vec3 ro, vec3 rd, vec2 tor) {
  ...
  if(h > 0.0) {
    h = sqrt(h);
    float t1 = d1 - h - k3;  t1 = (po<0.0)?2.0/t1:t1;
    float t2 = d1 + h - k3;  t2 = (po<0.0)?2.0/t1:t1;
...
vec3 color(vec2 p, vec2 q) {
  ...
  vec3 lp1 = ro;
  lp1.xy  *= ROT(0.85 + iAudio.w * 0.1); // Audio: Camera wobble
  lp1.xz  *= ROT(-0.5);
  ...
  // Audio: Torus minor radius pulses with bass
  vec2 tor = 0.55*vec2(1.0, 0.75 * (1.0 + iAudio.x * 0.5));
  ...
  // Audio: Stripe pattern animates/swirls with mids
  float a     = atan(tpos.y, tpos.x)-PI*tpos.z/(r+0.5*abs(tpos.z))-TTIME/45.0 - iAudio.y * 5.0;
  // Audio: Stripe width changes with treble
  float s     = mix(0.05, 0.5, tanh_approx(2.0*abs(td-0.75))) * (1.0 - iAudio.z * 0.5);
  // Audio: Colors react to mids
  vec3  bcol0 = vec3(0.3) + vec3(0.2, 0.1, 0.3) * iAudio.y;  
  vec3  bcol1 = vec3(0.025) + vec3(0.0, 0.1, 0.2) * iAudio.y;
