...
const source = `
// Worms from "Digiverse" by Fairlight (Worm/Pasta part only)
...
vec3 getObjectColor(vec3 p, vec3 n, vec2 mat, float time_in) {
    ...
    return vec3(.8 + iAudio.x * 0.2, .0, .4) + floor(.1 + fract(p.x * 14. - 3. * time_in));
}
...
`;
// FIX: Add default export to make the file a module.
