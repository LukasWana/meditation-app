// ComplicatedThings - CC0 by an anonymous author
// Adapted for Shader Sequencer: Fixed GLSL ES 1.00 compatibility and added audio reactivity.
...
const int   fadeFrom  = 8; // FIX: Replaced max(furthest-4, 0) which is not a constant expression in GLSL ES 1.00
const float fadeDist  = planeDist*float(furthest - fadeFrom);

// License: Unknown, author: Unknown, found: don't remember
vec4 alphaBlend(vec4 back, vec4 front) {
  float w = front.w + back.w*(1.0-front.w);
  vec3 xyz = (front.xyz*front.w + back.xyz*back.w*(1.0-front.w))/(w + 1e-6); // Added epsilon
...
float atan_approx(float y, float x) {
  float cosatan2 = x / (abs(x) + abs(y) + 1e-6); // Added epsilon
...
vec3 palette( float t ) {
  // Audio reactivity: mids (y) shift palette
  return (1.0+cos(1.0*vec3(0.0, 1.0, 2.0)+TAU*(t + iAudio.y * 0.5)))*0.5;
}
...
vec3 effect(vec2 p, float tm, float n) {
  ...
  vec2 kp = p;
  float kl = dot(kp, kp);
  // Audio reactivity: bass (x) changes kaleidoscope repetitions
  float nn = 2.0*floor(mix(3.0, 16.0, 0.5+0.5*sin(0.1*n + iAudio.x * 2.0)));
...
vec4 plane(vec3 ro, vec3 rd, vec3 pp, vec3 off, float aa, float n) {
  ...
  // Audio reactivity: highs (z) add shimmer/speed to rotation
  float tm = 0.5*0.125*TIME+0.125*n + iAudio.z * 0.1;
  p *= ROT(-tm);  
...
vec3 color(vec3 ww, vec3 uu, vec3 vv, vec3 ro, vec2 p) {
  ...
  for (int i = 1; i <= furthest; ++i) {
    ...
    if (pd > 0.0 && acol.w < cutOff) {
      ...
      vec4 pcol = plane(ro, rd, pp, off, aa, nz_base+float(i));

      float fade_z = pp.z-ro.z;
      float fadeIn = smoothstep(planeDist*float(furthest), planeDist*float(fadeFrom), fade_z);
      float fadeOut = smoothstep(0.0, planeDist*0.1, fade_z);
...
vec3 effect(vec2 p, vec2 pp) {
  // Audio reactivity: overall volume (w) affects master speed
  float tm  = planeDist*TIME*(BPM/60.0) * (1.0 + iAudio.w);
  vec3 ro   = offset(tm);
  vec3 dro  = doffset(tm);
